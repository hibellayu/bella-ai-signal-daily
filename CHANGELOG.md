...
2026/08/28 上午檢查時，正式站仍停在 2026/08/27。盤查後確認不是前台讀取錯誤，也不是內容生成失敗，而是 GitHub Actions 沒有出現 8/28 的主排程執行紀錄。由於 GitHub schedule 可能延遲或偶發漏跑，單一排程不足以支撐「每天自動更新」的使用期待。

### 修改內容

- 新增 `Daily AI Signal Watchdog` 補跑 workflow。
- 補跑 workflow 於台北時間 07:30 與 09:30 檢查當日日報 JSON 是否存在。
- 若當日日報已存在，補跑 workflow 會安靜跳過，不重複生成。
- 若當日日報不存在，補跑 workflow 會自動生成日報、更新首頁、每日靜態頁、日報列表與 `sitemap.xml`。
- 原本 `Daily AI Signal` 主 workflow 新增防重複機制：排程執行時若當日日報已存在，會跳過生成，避免延遲主排程覆蓋補跑內容。
- `Daily AI Signal` 手動執行新增 `force` 選項，必要時可針對已存在日期重新生成。
- Footer 版本升為 `v0.19.0`，版本日期更新為 2026/08/28。

### 驗證

- 檢查 2026/08/28 主排程未執行，本機與正式站 manifest 原停在 2026/08/27。
- 手動觸發 `Daily AI Signal` 產生 2026/08/28 日報：`33138764275`。
- 確認 GitHub Pages 部署成功：`33138978778`。
- 確認正式站首頁與 `/daily/2026-08-28/` 均可讀到 2026/08/28 日報。
- 待本版推送後，可手動執行 `Daily AI Signal Watchdog` 驗證「已存在就跳過」路徑。
